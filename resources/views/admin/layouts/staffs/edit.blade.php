...
@section('page-content')
    <div class="container-fluid px-4">
        ...
        <form action="{{ route('admin.staffs.update') }}" method="PATCH">
            @csrf
            <div class="from-group">
                <label for="title" class="form-label">老師名字： </label>
                <input id="name" name="name" value="{{old('name',$staff->name)}}">

                <p></p>
                <label for="content" class="form-label">介紹: </label>
                <textarea id="introduce" name="introduce" class="form-control" rows="10">{{old('introduce',$staff->introduce)}}</textarea>

                <p></p>
                <label for="content" class="form-label">上傳圖檔: </label>
                <input id="img_path" name="img_path" value="{{old('img_path',$staff->img_path)}}">
